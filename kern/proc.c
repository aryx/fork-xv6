...
pinit(void)
{
  ...
  //pad: the memset 0 for the bss would also implicitly reset
  // all the proc[] fields, and because UNUSED was 0, all
  // process entries were correctly set. If UNUSED is not 0 then 
  // need the following more explicit code:
...
// Return currently running process.  aliased via proc.h macro  cp
struct proc*
curproc(void)
{
  struct proc *p;
